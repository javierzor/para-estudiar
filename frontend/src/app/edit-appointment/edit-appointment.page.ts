...
export class EditAppointmentPage implements OnInit {
  ...
  constructor(
    ...
    this.firebaseserviciosservice.getBooking(this.id).valueChanges().subscribe(res => {
  ...
  updateForm() {
    this.firebaseserviciosservice.updateBooking(this.id, this.updateBookingForm.value)
      .then(() => {
